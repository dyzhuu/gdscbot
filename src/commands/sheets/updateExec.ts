...
export const data = new SlashCommandBuilder()
  ...
  .addStringOption((option) =>
    option
      .setName('name')
      .setDescription('Enter the name of the exec whose details to update')
      .setRequired(true)
  ...
  .addStringOption((option) =>
    option.setName('email').setDescription('Enter your preferred email address')
  )
  .addStringOption((option) =>
...
export async function execute(interaction: ChatInputCommandInteraction) {
  try {
    ...
    const fields: APIEmbedField[] = Object.entries(updatedExec)
      ...
      .map((x) => ({
        name: `${
          execPropertyFormatter[x[0] as keyof typeof execPropertyFormatter]
        }: `,
        value: x[1]
      }));
