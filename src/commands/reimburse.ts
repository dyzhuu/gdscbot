...
export async function executeButton(interaction: ButtonInteraction) {
  ...
  if (customId === 'approve') {
    ...
    newEmbed.setFooter({
      text: `Reimbursement approved by ${memberName} on ${interaction.createdAt.toLocaleString()}`
    });
    interaction.update({
      embeds: [newEmbed],
      components: [row]
    ...
    newEmbed.setImage(originalEmbed.image!.url);
    newEmbed.setFooter({
      text: `Reimbursement denied by ${memberName} on ${interaction.createdAt.toLocaleString()}`
    });
    interaction.update({
      embeds: [newEmbed],
      components: [row]
